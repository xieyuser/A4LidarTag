...
@click.command()
@click.option('--calib-config', default='./config/camera_l515_config.toml')
@click.option('--mono-calib', default="./config/mono_calib_l515.json")
@click.option('--l515', default="./config/l515_params.json")
@click.option('--pattern-dic', default="./config/redict.json")
@click.option('--out-f', default='./calib_result/camera_l515_transform.json')
@click.option('--verify-dir', default='./calib_result/verify')
def main(calib_config, mono_calib, l515, pattern_dic,  out_f, verify_dir):
    print("Calibrating...")

    with open(pattern_dic, 'r') as f:
        pattern_dic = edict(json.load(f))

    with open(l515, 'r') as f:
        l515 = edict(json.load(f))
    ...
    img_dir = C.mono.rectified_mono_dir
    pc_dir = C.lidar.pc_dir

    lidar_K = np.asarray(l515.lidar.K)
    lidar_w, lidar_h = l515.lidar.img_size

    corr_img_us, corr_lidar_Xs = [], []

    names = [file_name(i) for i in sorted(glob(osp.join(img_dir, "*.jpg")))]
    if True:
        for name in names:
            print(f"\nCalibrating {name} ...")
            img_f = osp.join(img_dir, name + '.jpg')
        ...
        np.save('data/corr_img_us_l515.npy', corr_img_us)
        np.save('data/corr_lidar_Xs_l515.npy', corr_lidar_Xs)
    else:
        corr_img_us = np.load('data/corr_img_us_l515.npy')
        corr_lidar_Xs = np.load('data/corr_lidar_Xs_l515.npy')
